...
# Create output directory for Standard engine
output_dir_standard = 'polly_audio_standard'
os.makedirs(output_dir_standard, exist_ok=True)

# Generate audio for each sentence using Standard engine
def generate_audio_standard(sentence, voice_id):
...
for voice in english_voices:
    for i, sentence in enumerate(sentences, start=1):
        file_name_standard = f"{output_dir_standard}/voice_{voice['Id']}_{i}.mp3"
        if not os.path.exists(file_name_standard):
            try:
                audio_stream = generate_audio_standard(sentence, voice['Id'])
                with open(file_name_standard, 'wb') as file:
                    file.write(audio_stream)
                print(f"Generated audio for sentence {i} with voice {voice['Id']} using Standard engine: {file_name_standard}")
            except Exception as e:
                print(f"Error generating audio for sentence {i} with voice {voice['Id']} using Standard engine: {e}")
        else:
            print(f"File {file_name_standard} already exists. Skipping generation.")

print("Standard engine audio generation complete.")
